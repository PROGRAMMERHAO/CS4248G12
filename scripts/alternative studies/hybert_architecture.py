...
from transformers import AlbertTokenizerFast, AlbertModel, AlbertForQuestionAnswering, AdamW, get_linear_schedule_with_warmup
...
model = AlbertLinear_highway(model_name=model_name)

# Load the pre-trained model's state_dict
pretrained_model = AlbertForQuestionAnswering.from_pretrained(model_name)
pretrained_state_dict = pretrained_model.state_dict()

# Get our model's state_dict
...
for name, param in pretrained_state_dict.items():
    ...
    elif name.startswith('qa_outputs.'):
        model_state_dict['qa_outputs.' + name[len('qa_outputs.'):]].copy_(param)

# Load the updated state_dict into our model
...
# Only include parameters that require gradients (highway layers and qa_outputs)
...
with open(os.path.join(eval_directory, 'fine_tuned_predictions.json'), 'w') as f:
