...
tokenizer = AlbertTokenizerFast.from_pretrained('albert-xlarge-v2')
model = AlbertForQuestionAnswering.from_pretrained('albert-xlarge-v2')
model.to(device)
...
dev_loader = DataLoader(dev_dataset, batch_size=12)

# Set up optimizer and scheduler
optimizer = AdamW(model.parameters(), lr=0.5e-5)
total_steps = len(train_loader) * 3  # 2 epochs
...
best_model_directory = '../models/fine_tuned_albert_xlarge_with_mix_model'
...
final_save_directory = '../models/fine_tuned_albert_xlarge_with_mix_model'
...
with open('../eval/fine_tuned_albert_xlarge_with_mix_predictions.json', 'w') as f:
    json.dump(predictions, f)

print('Predictions saved to fine_tuned_albert_xlarge_with_mix_predictions.json')
